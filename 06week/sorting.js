...
const stringsToNumbs = (numbers) => {return numbers.map(Number)}
...
const sumEvens = (numbers) => {
    ...
    console.log(evens.reduce((acc, curr) => acc + curr));
  }
sumEvens(stringsToNumbs(strNums));

// Find the index of the first value when added to it's index = 512 (#ATX!!)
let nums = stringsToNumbs(strNums);
...
const weatherStates = (weath) => {
    let weatherStateNames = [];
    weath.map(obj => {
        if (!weatherStateNames.includes(obj['weather_state_name'])){
          weatherStateNames.push(obj['weather_state_name']);
        }
    });
    return weatherStateNames;
  }
console.log("Weather State Names: " + weatherStates(weather));


//find the id of the object in weather that has a min_temp of 15.915
const idealTemp = (weath) => {
    weath.map(obj => {
       if(obj['min_temp'] == 15.915){
         console.log("id for temp at 15.915: " + obj['id']);   
       }
    })
  }
